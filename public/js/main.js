(() => {
  ...
  const initAuthSignup = () => {
    ...
    const syncIdFieldUi = (btn, filled) => {
      ...
      const placeholder = btn.querySelector(".auth-signup-email-page__id-input-placeholder");
      const value = btn.querySelector(".auth-signup-email-page__id-input-value");
      if (placeholder) placeholder.hidden = filled;
      if (value) value.hidden = !filled;
    };
    ...
    const syncKeyboardStickyUi = () => {
      if (window.__hybridSignup?.isActive?.() && window.__hybridSignup?.isManagedStep?.()) {
        window.__hybridSignup.syncSignupKeyboardMode?.();
        return;
      }
    ...
    const getSignupFlowStepIndex = () => {
      ...
      if (
        ...
        emailPage?.classList.contains("is-hybrid-mobile-code-active") ||
    ...
    if (emailPage) {
      ...
      window.__hybridSignupAdvanceNationality = () => showNationalityStepUi();
      ...
      const handleKeyboardPrimaryAction = () => {
